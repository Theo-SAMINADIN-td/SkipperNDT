...
class CurrentIntensityClassifier(nn.Module):
    ...
    def __init__(self, dropout_rate=0.1):
        ...
        self.resnet.fc = nn.Sequential(
            nn.Dropout(p=dropout_rate), nn.Linear(num_ftrs, 1)
        )
...
def find_best_threshold(model, dataloader, device, min_recall=0.85):
    ...
    for threshold in np.arange(0.2, 0.71, 0.05):
        ...
        print(
            f"    threshold={threshold:.2f} → Acc={acc:.4f} | Recall={rec:.4f} | F1={f1:.4f}{marker}"
        )
...
def plot_training_history(history, freeze_epochs=0, save_path="training_history.png"):
    ...
    for ax, train_key, val_key, title, ylabel, target in [
        ...
        if target:
            ax.axhline(
                y=target, color="r", linestyle="--", label=f"Target {target:.0%}"
            )
        if freeze_epochs > 0:
            ax.axvline(
                x=freeze_epochs - 1, color="orange", linestyle=":", label="Unfreeze"
            )
        ax.set_title(title)
        ax.set_xlabel("Epoch")
...
def load_data_with_labels(data_dir, csv_path=None):
    """
    ...
    if label_column not in df.columns:
        ...
        if "label" in df.columns:
            ...
            raise ValueError(
                f"No suitable label column found. Available: {available_cols}"
            )
    ...
    print(
        f"  ✓ {len(file_paths)} samples loaded (column: '{label_column}') | {missing} files missing"
    )
...
if __name__ == "__main__":
    ...
    print("\n1. Loading data...")
    file_paths, labels = load_data_with_labels(DATA_DIR, CSV_PATH)
    print(
        f"  Total: {len(file_paths)} | "
        f"Insufficient: {sum(1 for label in labels if label == 0.0)} | "
        f"Sufficient: {sum(1 for label in labels if label == 1.0)}"
    )
    ...
    num_insufficient = sum(1 for label in train_labels if label == 0.0)
    num_sufficient = sum(1 for label in train_labels if label == 1.0)
    pos_weight = 1.0
    print(
        f"  pos_weight: {pos_weight} (neutral) | "
    ...
    print(
        f"  Phase 2: {NUM_EPOCHS - FREEZE_EPOCHS} epochs — full fine-tune (lr={LR_FULL})"
    )
    ...
    for epoch in range(NUM_EPOCHS):
        ...
        print(
            f"Val   Loss: {val_loss:.4f} | Val Acc: {val_acc:.4f} | Recall: {val_recall:.4f} | F1: {val_f1:.4f}"
        )
        ...
        if combined_score > best_score and val_recall >= 0.85:
            ...
            print(
                f"  ✓ Model saved (score={combined_score:.4f} | Recall={val_recall:.4f} | F1={val_f1:.4f})"
            )
    ...
    if not best_model_name:
        raise RuntimeError(
            "No model was saved — val_recall never reached 0.85 during training."
        )
    ...
    print(
        f"Accuracy           : {test_acc:.4f}  (Target: > 0.90)  {'✓' if test_acc > 0.90 else '✗'}"
    )
    print(
        f"Recall             : {test_recall:.4f}  (Target: > 0.85)  {'✓' if test_recall > 0.85 else '✗'}"
    )

...
class PipeTypeClassifier(nn.Module):
    ...
    def load_backbone_from_presence_model(self, presence_model_path, device):
        if not os.path.exists(presence_model_path):
            raise FileNotFoundError(f"Modèle de présence introuvable : {presence_model_path}")

        checkpoint     = torch.load(presence_model_path, map_location=device, weights_only=False)
...
def plot_training_history(history, save_path):
    ...
    axes[1, 0].set_title('Recall - Parallel Pipes') ; axes[1, 0].legend() ; axes[1, 0].grid(True)

    axes[1, 1].plot(history['val_f1'], label='Val F1')
    axes[1, 1].set_title('F1-Score') ; axes[1, 1].legend() ; axes[1, 1].grid(True)
...
def main():
    ...
    print(f"  Augmentation activée sur le train set (hflip + vflip + rotation ±30°)")
    ...
    for epoch in range(NUM_EPOCHS):
        ...
        for k, v in zip(history.keys(), [train_loss, val_loss, train_acc, val_acc, val_recall, val_f1]):
            history[k].append(v)
    ...
    model.load_state_dict(
        torch.load(BEST_TYPE_MODEL_PATH, map_location=device, weights_only=False)['model_state_dict']
    )
    ...
    print(classification_report(test_true, test_preds, target_names=['Single Pipe', 'Parallel Pipes']))

...
def main():
    ...
    print(
        f"{len(npz_files)} fichiers NPZ | Resolution: {RESOLUTION_M_PER_PX} m/pixel\n"
    )
    ...
    print(
        f"MAE  : {mae:.4f}m  {'OBJECTIF ATTEINT' if mae < 1.0 else 'OBJECTIF NON ATTEINT'}"
    )
    ...
    axes[0].set_xlabel("Reel (m)")
    axes[0].set_ylabel("Predit (m)")
